...
def detect_battles(session, year: int, race_name: str,
    ...
    v5 (IP04) additions:
      - attacker_team / defender_team / same_team
    ...
    for lap_number in range(start_lap, max_lap + 1):
        ...
        for i in range(len(drivers_by_position) - 1):
            ...
            # ── IP04 §1.1: team / constructor context ────────────
            ...
            # ── IP04 §1.3: race situation beyond the local pair ──
            gap_to_car_ahead = ffu.gap_to_position(
                lap_data, defender_pos, defender_pos - 1
            ) if defender_pos > 1 else 99.0
            gap_to_car_behind = ffu.gap_to_position(
                lap_data, attacker_pos, attacker_pos + 1
            )
            drs_train_size = ffu.count_cars_within_drs(
                lap_data, attacker_pos, threshold=1.0
            )
            ...
            # ── IP04 §1.3: stint phase bucket (based on attacker tyre age) ──
